...
const LoginScreen: React.FC<LoginScreenProps> = ({ navigation }) => {
  ...
  // Context
  const { checkTwoFactorStatus } = useAuth()
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const loginResponse = await authService.login({ login: loginToSend, senha })
      const {
        token,
        user,
        session,
        twoFactorRequired: responseTwoFactorRequired,
        twoFactorToken: responseTwoFactorToken,
        message,
      } = loginResponse

      const requiresTwoFactor = Boolean(
        responseTwoFactorRequired ||
        has2FA ||
        user?.two_fa ||
      ...
      if (!token || !user) {
        throw new Error(message || "Token de autenticação não retornado pelo servidor.")
      }
