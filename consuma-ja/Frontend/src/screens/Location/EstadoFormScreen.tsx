...
type EstadoFormScreenProps = {
  navigation: any
  route: { params?: { estadoParaEditar?: Estado } }
}

const EstadoFormScreen: React.FC<EstadoFormScreenProps> = ({ route, navigation }) => {
  const estadoParaEditar = route.params?.estadoParaEditar
  const isEditing = Boolean(estadoParaEditar)

  const [nome, setNome] = useState('')
  const [sigla, setSigla] = useState('')
  const [loading, setLoading] = useState(false)
  const [errors, setErrors] = useState<{ nome?: string; sigla?: string }>({})

  useEffect(() => {
    if (isEditing && estadoParaEditar) {
      setNome(estadoParaEditar.estado_nome || '')
      setSigla(estadoParaEditar.estado_sigla || '')
      navigation.setOptions({ title: 'Editar Estado' })
    } else {
  ...
  const validarCampos = () => {
    const newErrors: { nome?: string; sigla?: string } = {}
  ...
  const handleSubmit = async () => {
    ...
    const estadoData = {
      ...
      estado_sigla: sigla.trim().toUpperCase(),
    }
    ...
    try {
      ...
      console.error('[EstadoFormScreen] Erro ao salvar estado:', err)
      const defaultMessage = isEditing ? 'Não foi possível atualizar o estado.' : 'Não foi possível criar o estado.'
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        ...
        disabled={loading}
      >
        {loading ? <ActivityIndicator size="small" color="#fff" /> : <Text style={styles.buttonText}>{isEditing ? 'Salvar Alterações' : 'Cadastrar Estado'}</Text>}
      </TouchableOpacity>
    </View>
  )
}
