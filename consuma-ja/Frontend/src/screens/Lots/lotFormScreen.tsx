...
import { lotFormStyles } from "../../common/styles/Lots/lotFormScreen.styled"
...
const LotFormScreen = () => {
  ...
  if (loading && isEditing) {
    return <ActivityIndicator size="large" style={lotFormStyles.centered} />
  }

  return (
    <ScrollView
      style={lotFormStyles.container}
      contentContainerStyle={lotFormStyles.contentContainer}
      keyboardShouldPersistTaps="handled"
    >
      <Text style={lotFormStyles.title}>{isEditing ? "Editar Lote" : "Novo Lote"}</Text>

      {/* Fornecedor (opcional, para filtrar produtos) */}
      <View style={lotFormStyles.formGroup}>
        <Text style={lotFormStyles.label}>Fornecedor (opcional, para filtrar produtos):</Text>
        {fornecedorSelecionado ? (
          <View style={lotFormStyles.selectedItem}>
            <Text style={lotFormStyles.selectedItemText}>{fornecedorSelecionado.pessoa_nome}</Text>
            <TouchableOpacity onPress={removerFornecedor} style={lotFormStyles.removeButton}>
              <Ionicons name="close-circle" size={24} color="#dc3545" />
            </TouchableOpacity>
          </View>
        ) : (
          <>
            <View style={lotFormStyles.searchContainer}>
              <TextInput
                style={lotFormStyles.searchInput}
              ...
              />
              <TouchableOpacity
                style={[lotFormStyles.searchButton, loadingFornecedores && lotFormStyles.buttonDisabled]}
              ...
              >
                {loadingFornecedores ? (
                  ...
                  <Text style={lotFormStyles.searchButtonText}>Buscar</Text>
                )}
              </TouchableOpacity>
            </View>
          </>
        ...
        {showFornecedores && (
          <View style={lotFormStyles.resultsContainer}>
            <ScrollView style={lotFormStyles.resultsList} nestedScrollEnabled={true}>
              {fornecedores.map((item) => (
                <TouchableOpacity
                  key={item.pessoa_id.toString()}
                  style={lotFormStyles.resultItem}
                  onPress={() => selecionarFornecedor(item)}
                >
                  <Text style={lotFormStyles.resultItemText}>{item.pessoa_nome}</Text>
      ...
      {/* Produto */}
      <View style={lotFormStyles.formGroup}>
        <Text style={lotFormStyles.label}>Produto:</Text>
        {produtoSelecionado ? (
          <View style={lotFormStyles.selectedItem}>
            <Text style={lotFormStyles.selectedItemText}>{produtoSelecionado.produto_nome}</Text>
            <TouchableOpacity onPress={removerProduto} style={lotFormStyles.removeButton} disabled={isEditing}>
          ...
          <>
            <View style={lotFormStyles.searchContainer}>
              <TextInput
                style={[lotFormStyles.searchInput, errors.produto ? lotFormStyles.inputError : null]}
                value={produtoQuery}
              ...
              />
              <TouchableOpacity
                style={[lotFormStyles.searchButton, (loadingProdutos || isEditing) && lotFormStyles.buttonDisabled]}
            ...
            {errors.produto ? <Text style={lotFormStyles.errorText}>{errors.produto}</Text> : null}
          </>
        )}
          ...
          <View style={lotFormStyles.resultsContainer}>
            <ScrollView style={lotFormStyles.resultsList} nestedScrollEnabled={true}>
              {produtos.map((item) => (
                ...
                >
                  <Text style={lotFormStyles.resultItemText}>{item.produto_nome}</Text>
                  <Ionicons name="checkmark-circle" size={24} color="#28a745" />
                </TouchableOpacity>
              ))}
            </ScrollView>
      ...
      {/* Código do Lote */}
      <View style={lotFormStyles.formGroup}>
        <Text style={lotFormStyles.label}>Código do Lote:</Text>
        <TextInput
          style={[lotFormStyles.input, errors.lote_codigo ? lotFormStyles.inputError : null]}
        ...
        />
        {errors.lote_codigo ? <Text style={lotFormStyles.errorText}>{errors.lote_codigo}</Text> : null}
      </View>

      {/* Data de Validade */}
      <View style={lotFormStyles.formGroup}>
        <Text style={lotFormStyles.label}>Data de Validade:</Text>
        <TouchableOpacity style={lotFormStyles.dateInput} onPress={() => mostrarDatePicker("validade")}>
      ...
      {/* Quantidade Inicial */}
      <View style={lotFormStyles.formGroup}>
        <Text style={lotFormStyles.label}>Quantidade Inicial:</Text>
        <TextInput
          style={[lotFormStyles.input, errors.lote_quantidade_inicial ? lotFormStyles.inputError : null]}
        ...
        />
        {errors.lote_quantidade_inicial ? <Text style={lotFormStyles.errorText}>{errors.lote_quantidade_inicial}</Text> : null}
      </View>

      {/* Quantidade Atual */}
      <View style={lotFormStyles.formGroup}>
        <Text style={lotFormStyles.label}>Quantidade Atual:</Text>
        <TextInput
          style={[lotFormStyles.input, errors.lote_quantidade_atual ? lotFormStyles.inputError : null]}
          value={formData.lote_quantidade_atual}
          onChangeText={(text) => setFormData((prev) => ({ ...prev, lote_quantidade_atual: text }))}
          keyboardType="numeric"
          placeholder="Ex: 100"
        />
        {errors.lote_quantidade_atual ? <Text style={lotFormStyles.errorText}>{errors.lote_quantidade_atual}</Text> : null}
      </View>

      {/* Data de Entrada */}
      <View style={lotFormStyles.formGroup}>
        <Text style={lotFormStyles.label}>Data de Entrada:</Text>
        <TouchableOpacity
          style={lotFormStyles.dateInput}
      ...
      <View style={lotFormStyles.formGroup}>
        <Text style={lotFormStyles.label}>Status:</Text>
        <View style={lotFormStyles.switchContainer}>
          <Text style={lotFormStyles.switchLabel}>{formData.ativo ? "Ativo" : "Inativo"}</Text>
          <Switch
      ...
      {/* Botão Salvar */}
      <TouchableOpacity
        style={[lotFormStyles.submitButton, loadingSubmit && lotFormStyles.buttonDisabled]}
      ...
      >
        {loadingSubmit ? (
          ...
          <Text style={lotFormStyles.submitButtonText}>{isEditing ? "Atualizar Lote" : "Salvar Lote"}</Text>
        )}
      </TouchableOpacity>
